...
def step_impl(context):
    context.execute_steps(f'''
        ...
        And I press "Log in"
    ''')
    context.browser.find_by_text('My Trello board')
    context.browser.find_by_css('.board-tile').click()

@then('I can see the three cards on the Trello board')
def step_impl(context):
    context.browser.find_by_text('Card 1')
    context.browser.find_by_text('Card 2')
    context.browser.find_by_text('Card 3')
...
def step_impl(context):
    ...
    context.browser.find_by_text('This is the card description')  
...
def step_impl(context):
    context.browser.is_text_not_present('Card 3')
...
def step_impl(context):
    context.browser.find_by_css('.badge').find_by_text('Comments')
...
def step_impl(context):
    context.browser.find_by_text('This is the browser card comment')
...
def step_impl(context):
    ...
    context.browser.find_by_text('in list Done')


@then('I should see there is no account for this username error message')
def step_impl(context):
    context.browser.find_by_text('There isn\'t an account for this username')
